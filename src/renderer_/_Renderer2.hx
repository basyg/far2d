...
class Renderer2 {
	...
	public var maxIndicesN(default, null):Int = 196608;
	...
	var _batchTrianglesN:Int = 0;
	var _batchVerticesN:Int = 0;
	...
	public function new(context3d:Context3D, width:Int, height:Int) {
		...
		_indexVector = new Vector(maxIndicesN, true);
		_indexBuffer.uploadFromVector(_indexVector, 0, maxIndicesN);
	...
	public function renderMesh(indices:Vector<UInt>, vertices:Vector<Float>, transform:Matrix, color:Int = 0xFFFFFF, alpha:Float = 1.0, textureName:String, xInTexture:Float, yInTexture:Float):Void {
		...
		if (_textureIds.exists(textureName)) {
			var indexBufferOffset = _batchTrianglesN * TRIANGLE_INDICES_N;
			var vertexBufferOffset = _batchVerticesN * BUFFER_VERTEX_SIZE;
			
			if (indexBufferOffset + trianglesN * TRIANGLE_INDICES_N > maxIndicesN || vertexBufferOffset + verticesN * BUFFER_VERTEX_SIZE > maxVerticesN) {
				flush();
				
				indexBufferOffset = _batchTrianglesN * TRIANGLE_INDICES_N;
				vertexBufferOffset = _batchVerticesN * BUFFER_VERTEX_SIZE;
			}
			
			var indexOffset = _batchVerticesN;
			var indexVector = _indexVector;
			for (i in 0...trianglesN) {
				var j = i * TRIANGLE_INDICES_N;
				indexVector[indexBufferOffset + j + 0] = indexOffset + indices[j + 0];
				indexVector[indexBufferOffset + j + 1] = indexOffset + indices[j + 1];
				indexVector[indexBufferOffset + j + 2] = indexOffset + indices[j + 2];
			}
			
			var vertexVector = _vertexVector;
			for (i in 0...verticesN) {
				var bufferI = i * BUFFER_VERTEX_SIZE;
				var meshI = i * MESH_VERTEX_SIZE;
				vertexVector[vertexBufferOffset + bufferI + 0] = vertices[meshI + 0];
				vertexVector[vertexBufferOffset + bufferI + 1] = vertices[meshI + 1];
			}
			...
			_batchSize++;
			_batchTrianglesN += trianglesN;
			_batchVerticesN += verticesN;
		}
	}
